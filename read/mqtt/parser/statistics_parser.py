...
class StatisticsParser(ParserInterface):
    ...
    def deserialize(self, payload):
        ...
        if message_to_dict:
            ...
            client_data = StatisticsDto(message_to_dict.serial_num, message_to_dict.device_id, message_to_dict.mac_addr, message_to_dict.clients)
            logger.info("Statistics clients data  %s", client_data)
            client.connected_client.create_client(client_data)
            
            device_data = StatisticsDto(message_to_dict.serial_num, message_to_dict.device_id, message_to_dict.mac_addr, message_to_dict.device)
            logger.info("Statistics device data  %s", device_data)
            client.device.create_device(device_data)
            
            vif_data = StatisticsDto(message_to_dict.serial_num, message_to_dict.device_id, message_to_dict.mac_addr, message_to_dict.vif)
            logger.info("Statistics vif data  %s", vif_data)
            client.vif.create_vif(vif_data)
            
            neighbors_data = StatisticsDto(message_to_dict.serial_num, message_to_dict.device_id, message_to_dict.mac_addr, message_to_dict.neighbors)
            logger.info("Statistics neighbors data  %s", message_to_dict.neighbors_data)
