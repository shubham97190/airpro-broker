...
from read.mqtt.dto.client_dto import DeviceDto, StatisticsDto
...
class StatisticsParser(ParserInterface):
    ...
    def deserialize(self, payload):
        ...
        if message_to_dict:
            ...
            logger.info("Data Received for serialNum [%s] Device Id [%s] mac_addr [%s]", serial_num, device_id, mac_addr)
            data = DeviceDto(serial_num, device_id, mac_addr).json
            response = db_client_factory.get_db_client('base-device').create(self, data)
            ...
            for key in self.creators:
                try:
                    ...
                    if data_to_push:
                        data = StatisticsDto(response['data']['id'], data_to_push).json
